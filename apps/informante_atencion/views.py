...
class AgregarInformanteAtencionView(APIView):
    ...
    def post(self, request, id_personal):
        ...
        try:
            # Verificar si el id_personal existe
            PersonalMedico.objects.get(pk=id_personal)
        except PersonalMedico.DoesNotExist:
            return Response("El id_personal no existe.", status=status.HTTP_404_NOT_FOUND)

        request.data["id_personal"] = id_personal
